  ...
  const handleVrifyKyc = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/acceptOrDeclineKyc",
        {
          orgId: orgId,
          address: citizenAddr,
    ...
    <div className="flex gap-4 mt-10">
      ...
      <div>
        <div className="p-4 max-w-md mx-auto bg-white rounded-xl shadow-md space-y-4">
          ...
          <div>
            <p className="text-gray-700">
              <span className="font-semibold">Time:</span> {formattedDate}
            </p>
          </div>
        </div>
      ...
      <div className="w-full">
        <h3 className="text-center font-semibold text-2xl">
          Citizens e-KYC (NID Image)
        </h3>
        <div className="mb-4 mt-4">
          <h3 className="font-bold">Front Side</h3>
          <img
            src={`${gatewayBaseUrl}/${ipfsHash}/images-${nid.nid}/image1.jpg`}
            alt="Image 1"
        ...
        <div>
          ...
          <img
            src={`${gatewayBaseUrl}/${ipfsHash}/images-${nid.nid}/image2.jpg`}
            alt="Image 2"
            className="mx-auto h-[300px] w-[500px]"
          />
        </div>
